...
var controllers = {
    ...
    get_product_array: function (req, res) {
        if (!req.session.product_array_all) {
            console.log("product_array_all không có");
            ...
            console.log("product_array_all có");
        }
    },

    get_media_array: function (req, res) {
        if (!req.session.media_array_all) {
            console.log("media_array_all không có");
    ...
    get_industry_array: function (req, res) {
        if (!req.session.industry_array_all) {
            ...
            console.log("industry_array_all có");
        }
    },

    get_location_array: function (req, res) {
        if (!req.session.location_array_all) {
            console.log("location_array_all không có");
            ...
            console.log("location_array_all có");
        }
    },
    ...
    get_store_detail: function (req, res) {
        ...
        } else if (!req.param("id") && req.session.store_id_current) {
            id_store = req.session.store_id_current;
        } else if (!req.session.store_id_current) {
            //500 nè, bắn qua index
        }
    ...
    get_product_detail: function (req, res) {
        ...
        } else if (!req.param('id') && req.session.product_id_current) {
            product_id = req.session.product_id_current;
        }
        req.session.product_id_current = product_id;
        product_schema.product.find({_id: product_id}, function (product_error, product_array) {
            if (product_array && product_array.length > 0) {
                req.session.product_array = product_array;
                var media_query = media_schema.media.find({product_id: product_id});
                media_query.sort({media_date: -1});
                media_query.exec(function (media_error, media_array) {
    ...
    get_insert_media: function (req, res) {
        ...
        if (req.param('id')) {
            console.log("1");
            req.session.product_id_current = req.param('id');
            ...
            console.log("3");
            controllers.back_index(req, res);
        }
    },
    ...
    get_search: function (req, res) {
        ...
        if (req.session.header == true) {
            console.log("có nha ");
            console.log(req.session.store_array_header_search);
            res.render('search', {store_array: req.session.store_array_header_search, industry_array: req.session.industry_array_all, location_array: req.session.location_array_all});
        } else if (req.session.header == false) {
            res.render('search', {store_array: req.session.store_array_header_search, industry_array: req.session.industry_array_all, location_array: req.session.location_array_all, search_notification: "Không có cửa hàng nào cả. :("});
        } else if (!req.session.header) {
            location_schema.location.find(function (location_error, location_array) {
                if (!location_error && location_array.length > 0) {
                    req.session.location_array = location_array;
                    var industry_query = industry_schema.industry.find({});
            ...
            if (key == "" && district == "Tất Cả Các Quận") { //tìm tất cả các store
                ...
                store_query.exec(function (store_error, store_array) {
                    if (store_array && store_array.length > 0) {
                        ...
                        res.render('search', {store_array: store_array, industry_array: req.session.industry_array_all, location_array: req.session.location_array_all});
                    } else {
                        console.log("1 - không có.");
                        res.render('search', {store_array: store_array, industry_array: req.session.industry_array_all, location_array: req.session.location_array_all, search_notification: "Không có cửa hàng nào cả. :("});
                    }
                });
            ...
            if (district == "Tất Cả Các Quận") { //Không có quận.
                ...
                product_schema.product.find({$or: [
                    ...
                    if (!product_error && product_array && product_array.length > 0) {
                        res.render('search', {product_array: product_array, industry_array: req.session.industry_array_all, location_array: req.session.location_array_all});
                    } else {
                        console.log("4-1 Không có.");
                        res.render('search', {product_array: product_array, industry_array: req.session.industry_array_all, location_array: req.session.location_array_all, search_notification: "Không có cửa hàng nào cả. :("});
                    }
                });
            } else { //Có quận.
                console.log("4-2");
                ...
                store_query.exec(function (store_error, store_array) {
                    ...
                    setTimeout(function () {
                        ...
                        if (product_array_render && product_array_render.length > 0) {
                            res.render('search', {product_array: product_array_render, industry_array: req.session.industry_array_all, location_array: req.session.location_array_all});
                        } else {
                            console.log("4-2 Không có.");
                            res.render('search', {product_array: product_array_render, industry_array: req.session.industry_array_all, location_array: req.session.location_array_all, search_notification: "Không có cửa hàng nào cả. :("});
                        }
                    }, 20);
                });
            }
    ...
    get_test: function (req, res) {
        controllers.get_product_array(req, res);
        setTimeout(function () {
            console.log(req.session.product_array_all);
            res.render('test', {product_array: req.session.product_array_all});
        }, 900)
    },
...
module.exports = function (router) {
    ...
    router.post('/header/search/:key', controllers.header_search);
